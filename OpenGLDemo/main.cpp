...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model_shader.setMat4("model", glm::mat4());
        //light configure
        glm::vec3 lightColor;
        lightColor.x = 1.0f;
        lightColor.y = 1.0f;
        ...
        model_shader.setVec3("light.diffuse", diffuseColor);
        model_shader.setVec3("light.specular", lightColor);

        if (lightPos.y < 0 && down)
            down = !down;
        if (lightPos.y > 15 && !down)
            down = !down;

        if (down)
            lightPos.y -= deltaTime * 2;
        else
            lightPos.y += deltaTime * 2;

        model_shader.setVec3("light.position", lightPos);
